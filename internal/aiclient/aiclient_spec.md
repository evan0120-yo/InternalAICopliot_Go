...
在第一版 promptguard integration 中，AIClient 也會承接 promptguard service 的第二層 LLM guard call；這條 path 應與主 consult analyze 分開，使用獨立的 promptguard startup config 與 dedicated guard JSON contract。
...
- 對 promptguard service 暴露 dedicated guard analyze 入口
...
- error mapping
- promptguard cloud/local routing
- dedicated guard JSON parse
...
promptguard service
      ...
      │
      ├─ send dedicated guard prompt
      ├─ parse dedicated guard JSON
      │   ├─ status=true  -> allow
      │   └─ status=false -> block
      └─ transport / parse / provider failure -> system error
```
...
## PromptGuard Response Contract
promptguard path 應使用 dedicated guard JSON contract，而不是沿用 consult business response：

```json
{
  "status": true,
  "statusAns": "SAFE",
  "reason": "guard reasoning"
}
```
...
- `statusAns` 可承接固定 guard 判斷摘要。
- `reason` 只提供 promptguard / gatekeeper 做內部判讀，不應被誤當成主分析回覆文字。
...
## PromptGuard Provider Contract
第一版 promptguard 的第二層 LLM guard 應走 Gemma family，而不是沿用主分析路徑的 OpenAI/Gemma provider 抽象。

規則：
- `cloud` 模式打 hosted Gemma。
- `local` 模式打 local Gemma endpoint。
- 這條 path 的 request/response contract 應以 guard JSON 為主，而不是 consult business response schema。
- promptguard path 不處理附件 upload，也不需要套用主 consult 的 Files API 邏輯。

...
func TestAnalyzeLiveModeDirectGemmaSupportsStructuredExtractionContract(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok || properties["taskType"] == nil || properties["eventId"] == nil || properties["startAt"] == nil || properties["queryStartAt"] == nil || properties["queryEndAt"] == nil || properties["missingFields"] == nil {
			...
			"candidates": []map[string]any{
				{
					"content": map[string]any{
						"parts": []map[string]any{
							{
								"text": `{"taskType":"calendar","operation":"create","eventId":"","summary":"找小傑吃飯","startAt":"2026-04-15 15:00:00","endAt":"2026-04-15 15:30:00","queryStartAt":"","queryEndAt":"","location":"","missingFields":[]}`,
							},
						},
					},
				},
	...
	if !strings.Contains(response.Response, `"eventId":""`) || !strings.Contains(response.Response, `"queryStartAt":""`) || !strings.Contains(response.Response, `"queryEndAt":""`) {
		t.Fatalf("expected extraction JSON to include new fields, got %+v", response)
	}
}

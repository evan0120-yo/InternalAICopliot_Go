...
- `analysisType`
- `theoryVersion` optional, required when the analysis type uses Internal-side theory translation
- `payload`

`ProfileConsult` 補充規則：
- `subjectProfile` 可在 text-only profile request 中省略
- builder command 必須保留 explicit `ConsultModeProfile`
- LinkChat 的 `astrology` analysis type 屬於 theory-translation-enabled analysis，`theoryVersion` 必填
- LinkChat 的 `mbti` analysis type 目前不走 Internal-side theory translation，`theoryVersion` 可省略
...
- `ProfileConsult`、app-aware prompt strategy 與 `theoryVersion` 已落成 proto 與 production code
- LinkChat 的 theory translation 現已明確轉向「Internal 先翻譯成語意片段再送 AI」，不再把 Internal private code / THEORY_CODEBOOK 暴露給模型
...
theoryMappings.analysisScopeKey ───corresponds─→ LinkChat analysis factory 的理論範圍

...
│  ├─ 是 -> 跳過 promptguard
│  └─ 否 -> 依序對每段 candidate text 跑 promptguard
│
├─ promptguard usecase
│  │
│  ├─ text scoring
...
- 第一版 text scoring 已落成 rule-based classifier
- 一旦 promptguard 正式接管 text injection / override guard，main consult prompt 不應再重複執行這段檢查
- `promptguard` command 仍只看單一 candidate text
- current runtime 中 gatekeeper 會分別把 `userText` 與 transport 直接帶入的 `intentText` 各跑一次 promptguard
...
- `intentText`
  - 上游系統已決定好的任務意圖
  - 目標上應是 trusted
  - 但 current runtime 若直接由 HTTP / gRPC transport 帶入，仍必須交給 promptguard

補充：
- Internal backend 只需要支援 `intentText` 這種 input 形狀；是否真的 trusted 取決於上游是否先建立 preset-key 到文字的可信來源邊界
- LinkChat 或其他上游系統若用 preset key 決定任務意圖，應在上游先把 key 映射成 `intentText`
- 相容期內既有 `text` 欄位可暫時視為 `userText` alias
...
│  │ userText?          │  │  ← optional 自由輸入
│  │ intentText?        │  │  ← optional 任務意圖；是否 trusted 取決於上游來源
│  └────────────────────┘  │
└────────────┬─────────────┘
             ▼
     Internal gRPC 入口
...
- `POST /api/profile-consult` 使用 `application/json`，可帶 optional `appId`、`subjectProfile`、`userText` 與 `intentText`
  - 相容期內既有 `text` 可暫時視為 `userText` alias
...
`ProfileConsult` 補充規則：
- `subjectProfile` 可在 `userText`-only 或 `intentText`-only profile request 中省略
- `userText` 表示使用者自由輸入，應視為 untrusted
- `intentText` 的設計目標是 trusted upstream intent；但 current runtime 若由 transport 直接帶入，仍會交給 promptguard
